...
###### Acknowledgments ##########################
# -function train_gan training loop, flat_accuracy and format_time code was modified from a tutorial for fine0tuning Bert for Sequence classification:
...
# -function train_gan code was also influenced from the DeepConvolutional GAN tutorial on pytorch's website:
...
def train_gan(self, training_dataloader: DataLoader, validation_dataloader: DataLoader):
    ...
    for epoch_i in range(0, self.epochs):
        ...
        for step, batch in enumerate(training_dataloader):
            ...
            D_x, discriminator_real_loss = self.discriminator.train_discriminator(batch,
                                                                                  self.real_labels)  ### Adds the correct list of labels to the batch
            ...
            bert_sents = [self.generator.detokenize(tokenizer.convert_ids_to_tokens(sent.tolist())).split() for sent in
                          generated_input_ids]
            ...
            D_G_z1, discriminator_fake_loss = self.discriminator.train_discriminator(
                ...
                self.false_labels)  ### Adds the correct list of labels to the batch
            ...
            D_G_z2, generator_loss = self.generator.train_generator(generated_input_ids, self.real_labels,
                                                                    self.discriminator)
        ...
        for batch in validation_dataloader:
            with torch.no_grad():
                ...
                validation_sents = [self.generator.detokenize(tokenizer.convert_ids_to_tokens(sent.tolist())).split()
                                    for sent in generated_input_ids]
                with open(settings.get_bert_valid_out_path(), 'a+') as f:
                    [f.write('[%d/%d][%d/%d]\t' % (epoch_i, self.epochs, index, len(validation_sents)) + " ".join(
